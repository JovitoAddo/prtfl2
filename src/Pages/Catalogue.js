  ...
  return (
    <>
      <div className="flex justify-center">
        <div className="grid items-end w-full h-36 border-b-2 border-customblack bg-gradient-to-r from-customblue to-customred">
      ...
      <div className="md:flex grid gap-5 w-full justify-around p-5 bg-opacity-10 bg-customblack">
        <button className="border-2 px-6 py-2 border-none bg-customblue bg-opacity-60 text-customwhite rounded-sm"
        onClick={()=>{setCate("all")}}
        >
          all
        </button>
        <button className="border-2 px-6 py-2 border-none bg-customblue bg-opacity-60 text-customwhite rounded-sm"
        onClick={()=>{setCate("electronics")}}
        >
          electronics
        </button>
        <button className="border-2 px-6 py-2 border-none bg-customblue bg-opacity-60 text-customwhite rounded-sm"
        onClick={()=>{setCate("men's clothing")}}
        >
          men's Clothing
        </button>
        <button className="border-2 px-6 py-2 border-none bg-customblue bg-opacity-60 text-customwhite rounded-sm"
        onClick={()=>{setCate("jewelery")}}
        >
          jewelry
        </button>
      </div>
      ...
      {product.length > 0 ? (
        <div className="min-h-screen grid justify-items-center p-2 gap-8">
          {product
            .filter((produk) => produk.category == "men's clothing")
            .map((produk, index) => (
              <div key={index}>
                <catalog
                  as
                ...
                >
                  <div className="grid justify-items-center items-center prose bg-transparent">
                    <h1 className="text-center font-poiret bg-transparent cursor-default">
                      {produk.title}
                    </h1>
                    ...
                    <button
                      as
                      div
                      className="flex gap-3 items-end pb-2 bg-transparent"
                    >
                      <Link
                        to={`/catalogue/${produk.id}`}
